...
val_loss_coeff = 0.5
entropy_loss_coeff  = 0.01  #PPO entropy para

# Time Horizone for PPO
TimeHorizon  = 20
K_Epoch = 100
###############################################################################
############################### Class ReplayMemory ############################
...
class PPO:
    ...
    def __init__(self, observationSpace, actionSpace, numberOfNeurons=numberOfNeurons, dropout=dropout, 
        ...
        # Set the Four Deep Neural Networks of the PPO algorithm (actor and critic)
    ...
    def learning(self):
        ...
        if (len(self.replayMemory) >= batchSize):
            ...
            #v_next = self.v(nextState).squeeze(1)   # GAE

            # generate advacend estimation by GAE
            #advs = torch.zeros_like(reward, device=self.device)     # GAE
            ##next_return = torch.tensor(0.0, dtype = reward.dtype, device=self.device)

            #for t in reversed(range(len(reward))):                                       # n-step
             #   delta = reward[t] + gamma * v_next[t] * (1 - done[t]) - v_cur[t]  # GAE
             #   next_return = delta + gamma * lam * (1 - done[t]) * next_return   # GAE
             #   advs[t] = next_return                                             # GAE

            advs = returns + (gamma ** TimeHorizon) * v_terminal - v_cur   # n-STEP 
            #returns = advs + v_cur  # GAE
            ...
            for _ in range(1):
                ...
                policy_loss = -torch.min(sur_1, sur_2).mean() - entropy_loss_coeff * entropy_loss           
    ...
    def training(self, trainingEnv, trainingParameters=[],
        ...
        try:
            ...
            for episode in tqdm(range(trainingParameters[0]), disable=not(verbose)):                
                ...
                # PPO is on-policy, every trace using old policy need to be removed
                self.replayMemory.reset()
                if (episode + 1) % episodeSavePeriod == 0:
                    fileName = "".join(["Strategies/", "PPO", "_", str(marketSymbol),  "_", trainingEnv.startingDate, "_", trainingEnv.endingDate, "_", str(episode)])
       ...
       # rendoring loss
        fig = plt.figure()
        fig.suptitle('PPO Training Loss')
        ...
        plt.savefig(''.join(['Figures/', str(marketSymbol), '_PPO_TrainingLoss', '.png']))
        plt.close()    
        ...
        if plotTraining:
            ...
            plt.savefig(''.join(['Figures/', str(marketSymbol), '_PPO_TrainingTestingPerformance', '.png']))
            plt.close()
            #plt.show()
            for i in range(len(trainingEnvList)):
    ...
    def testing(self, trainingEnv, testingEnv, rendering=False, showPerformance=False, showComment="PPO"):
        """
    ...
    def plotTraining(self, score, marketSymbol):
        ...
        ax1.plot(score)
        plt.savefig(''.join(['Figures/', str(marketSymbol), '_PPO_TrainingResults', '.png']))
        plt.close()
        #plt.show()
    ...
    def plotVValues(self, VValues, marketSymbol):
        ...
        ax1.plot(VValues)
        plt.savefig(''.join(['Figures/', str(marketSymbol), '_PPO_VValues', '.png']))
        plt.close()
    ...
    def plotExpectedPerformance(self, trainingEnv, trainingParameters=[], iterations=10):
        ...
        for i in range(iteration):
            ...
            ax.legend(["Training", "Testing"])
            plt.savefig(''.join(['Figures/', str(marketSymbol), '_PPO_TrainingTestingPerformance', str(i+1), '.png']))
            plt.close()
        ...
        ax.legend(["Training", "Testing"])
        plt.savefig(''.join(['Figures/', str(marketSymbol), '_PPO_TrainingTestingExpectedPerformance', '.png']))
        plt.close()
