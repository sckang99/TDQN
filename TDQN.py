...
targetNetworkUpdate = 1000 # 10 ~ 1000 not much impact on the performance
...
numberOfEncoderLayers = 6
numberOfNeurons = 512
numberOfHeads = numberOfNeurons // 64
dropout = 0.1
...
class Transformer(nn.Module):
    ...
    def __init__(self, numberOfInputs, numberOfOutputs, numberOfHeads=8, numberOfEncoderLayers=6, numberOfNeurons=512, dropout=0.1):
        """
        ...
        self.state_dim = numberOfInputs
        self.action_dim = numberOfOutputs

        # Embedding layer to transform input size to the model size
        self.embedding = nn.Linear(numberOfInputs, numberOfNeurons)
        ...
        encoder_layers = nn.TransformerEncoderLayer(numberOfNeurons, numberOfHeads, numberOfNeurons, dropout)
        
        # Stack multiple encoder layers to create a transformer encoder
        self.transformer_encoder = nn.TransformerEncoder(encoder_layers, numberOfEncoderLayers)
        
        # Define the final linear layer that maps from the transformer output to the desired output size
        self.fc_out = nn.Linear(numberOfNeurons, numberOfOutputs)

    def forward(self, input):
        x = self.embedding(input)
        y = self.pos_encoder(input)
        combined = x + y
        x = self.transformer_encoder(combined)
...
class TDQN:
    ...
    VARIABLES:  - model TRANSFORMER or LINEAR
    ...
    def __init__(self, observationSpace, actionSpace, numberOfHeds=numberOfHeads, numberOfEncoderLayers=numberOfEncoderLayers,
                 numberOfNeurons=numberOfNeurons, dropout=dropout, 
        ...
        if nn_model == 'TRANSFORMER':
            ...
            self.policyNetwork = Transformer(observationSpace, actionSpace, numberOfHeds, numberOfEncoderLayers, numberOfNeurons, dropout).to(self.device)
            self.targetNetwork = Transformer(observationSpace, actionSpace, numberOfHeds, numberOfEncoderLayers, numberOfNeurons, dropout).to(self.device)
        else:    
            self.policyNetwork = Linear(observationSpace, actionSpace, numberOfNeurons, dropout).to(self.device)
            self.targetNetwork = Linear(observationSpace, actionSpace, numberOfNeurons, dropout).to(self.device)
    ...
    def getNormalizationCoefficients(self, tradingEnv):
        ...
        # 5. Technical Analysis => minimum and maximum values
    ...
    def learning(self, batchSize=batchSize):
        ...
        if (len(self.replayMemory) >= batchSize):
            ...
            # Compute the next Q values returned by the target network - these exactly DDQN
            with torch.no_grad():
                ...
                for i in range(len(trainingEnvList)):
                    ...
                    while done == 0:
                        ...
                        if stepsCounter % learningUpdatePeriod == 0:
                            value_loss = self.learning()
                            loss_history.append(value_loss)
