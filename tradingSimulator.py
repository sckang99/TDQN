...
from extract_TensorBoard import extract_TensorBoard
...
stateLength = 30
observationSpace = 1 + (stateLength-1)*(4+len(technicalAnalysis))
actionSpace = 2

# Variables setting up the default transaction costs
percentageCosts = 0.1
transactionCosts = percentageCosts/100
...
class TradingSimulator:
    ...
    def displayTestbench(self, startingDate=startingDate, endingDate=endingDate, testDate=testDate):
        """
        ...
        for _, stock in indices.items():
            env = TradingEnv(stock, startingDate, endingDate, testDate, 0)
            env.render() 
    ...
    def analyseTimeSeries(self, stockName, startingDate=startingDate, endingDate=endingDate, splitingDate=splitingDate):           
        ...
        print("------------------------------------------\n")
        trainingEnv = TradingEnv(stock, startingDate, splitingDate, testDate, 0)
        ...
        print("------------------------------------------\n")
        testingEnv = TradingEnv(stock, splitingDate, endingDate, testDate, 0)
        ...
        print("---------------------------------------------------------------------\n")
        tradingEnv = TradingEnv(stock, startingDate, endingDate, testDate, 0)
    ...
    def plotEntireTrading(self, trainingEnv, testingEnv, showComment):
        """
        ...
        plt.savefig(''.join(['Figures/', str(trainingEnv.marketSymbol), '_', showComment, '_TrainingTestingRendering', '.png'])) 
        plt.close()
        #plt.show()
    ...
    def simulateNewStrategy(self, strategyName, stockName,
                            ...
                            saveStrategy=False, retraining=False):
        """
        ...
        trainingEnv = TradingEnv(stock, startingDate, splitingDate, endingDate, money, stateLength, transactionCosts)
        displayComment = "".join([strategy, "_", stock, "_", startingDate, "_", splitingDate])
        ...
        testingEnv = TradingEnv(stock, splitingDate, endingDate, endingDate, money, stateLength, transactionCosts)

        # Testing of the trading strategy
        testingEnv = tradingStrategy.testing(trainingEnv, testingEnv, rendering=rendering, showPerformance=showPerformance, showComment=displayComment)
            
        # Show the entire unified rendering of the training and testing phases
        if rendering:
            self.plotEntireTrading(trainingEnv, testingEnv, showComment=displayComment)
        ...
        # extract Sharpe Ratio at the end of the trining and testing
        extract_SR = extract_TensorBoard()
        extract_SR.extract_and_save()
    ...
    def simulateExistingStrategy(self, strategyName, stockName,
                                 ...
                                 observationSpace=observationSpace, actionSpace=actionSpace, 
                                 money=money, stateLength=stateLength, transactionCosts=transactionCosts,
                                 rendering=True, showPerformance=True, showAll=False):
        """
        ...
        # 2. LOADING PHASE    
        if(showAll):
            fileNames = "".join(["Strategies/", strategy, "_", stock, "_", startingDate, "_", splitingDate, "*"])
        else:
            fileNames = "".join(["Strategies/", strategy, "_", stock, "_", startingDate, "_", splitingDate])
        ...
        for fileName in files:
            exists = os.path.isfile(fileName)
            displayComment = fileName[fileName.find("\\")+1:]
            # If affirmative, load the trading strategy
            if exists:
                if ai:
                    strategyModule = importlib.import_module(strategy)
            ...
            # Testing of the trading strategy
            trainingEnv = tradingStrategy.testing(trainingEnv, trainingEnv, rendering=rendering, showPerformance=showPerformance, showComment=displayComment)
            testingEnv = tradingStrategy.testing(trainingEnv, testingEnv, rendering=rendering, showPerformance=showPerformance, showComment=displayComment)
