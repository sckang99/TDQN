...
class DDPG:
    ...
    def learning(self, batchSize=batchSize):
        ...
        if (len(self.replayMemory) >= batchSize):
            ...
            currentQValues = self.q(state, action.unsqueeze(1)).squeeze(1)
            
            # Compute the next Q values returned by the target network
            with torch.no_grad():
                nextActions = self.pi(nextState).max(1)
                nextQValues = self.q_target(nextState, nextActions).squeeze(1)
                expectedQValues = reward + gamma * nextQValues * (1 - done)

            # Compute the Huber loss
            value_loss = F.smooth_l1_loss(currentQValues, expectedQValues)
            ...
            policy_loss = -self.q(state, actions).mean()
    ...
    def training(self, trainingEnv, trainingParameters=[],
        ...
        try:
            ...
            for episode in tqdm(range(trainingParameters[0]), disable=not(verbose)):
                ...
                self.noise.scale = (noise_scale - final_noise_scale) * max(0, trainingParameters[0]-episode)/trainingParameters[0] + final_noise_scale
                ...
                for i in range(len(trainingEnvList)):
                    ...
                    while done == 0:
                        ...
                        # Execute the DDPG learning procedure
                        stepsCounter += 1
    ...
    def testing(self, trainingEnv, testingEnv, rendering=False, showPerformance=False):
        ...
        while done == 0:
            ...
            q0value = self.q(tensorState.unsqueeze(0), tensorAction.unsqueeze(0))
            tensorAction = torch.tensor(np.roll(action,1), dtype=torch.float, device=self.device)
            q1value = self.q(tensorState.unsqueeze(0), tensorAction.unsqueeze(0))
            ...
            QValues0.append(q0value.detach().numpy().squeeze(0))
            QValues1.append(q1value.detach().numpy().squeeze(0))
    ...
    def saveModel(self, qfileName, pfileName):
        """
        ...
        torch.save(self.q.state_dict(), qfileName)
        torch.save(self.pi.state_dict(), pfileName)
        


    def loadModel(self, qfileName, pfileName):
        """
        ...
        self.q.load_state_dict(torch.load(qfileName, map_location=self.device))
        self.q_target.load_state_dict(self.q.state_dict())
        self.pi.load_state_dict(torch.load(pfileName, map_location=self.device))
        self.pi_target.load_state_dict(self.pi.state_dict())
