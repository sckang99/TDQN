...
gamma = 0.4
policy_learningRate = 0.000001
value_learningRate = 0.000001
...
# Default soft update parameter tau=1.0 = same as hard update typicall value 0.001
tau = 1.0
...
class OrnsteinUhlenbeckActionNoise:   # if theta = 1, same as scale * N(mu, sigma)
    def __init__(self, action_space, mu = 0.0, sigma=0.3, theta=0.5, scale=1.0):
        self.theta = theta
...
class Actor(nn.Module):
    ...
    def __init__(self, numberOfInputs, numberOfOutputs, numberOfNeurons=numberOfNeurons, dropout=dropout):
        ...
        self.fc4 = nn.Linear(numberOfNeurons, numberOfOutputs)
        ...
        # Optimizer
        self.optimizer = optim.Adam(self.parameters(), lr=policy_learningRate, weight_decay=L2Factor)
    ...
    def forward(self, input):
        ...
        prob = F.softmax(self.fc4(x), dim=1)
        return prob
###############################################################################
################################### Class Critic #################################
###############################################################################
...
class Critic(nn.Module):
    ...
    def __init__(self, numberOfInputs, numberOfOutputs, numberOfNeurons=numberOfNeurons, dropout=dropout):
        ...
        # Definition of some Fully Connected layers
        self.fc1 = nn.Linear(numberOfInputs + numberOfOutputs, numberOfNeurons)
        self.fc2 = nn.Linear(numberOfNeurons, numberOfNeurons)
        self.fc3 = nn.Linear(numberOfNeurons, numberOfNeurons)
        self.fc4 = nn.Linear(numberOfNeurons, 1)
        ...
        # Definition of some Fully Connected layers
        self.fc6 = nn.Linear(numberOfInputs + numberOfOutputs, numberOfNeurons)
        self.fc7 = nn.Linear(numberOfNeurons, numberOfNeurons)
        self.fc8 = nn.Linear(numberOfNeurons, numberOfNeurons)
        self.fc9 = nn.Linear(numberOfNeurons, 1)

        # Definition of some Batch Normalization layers
        self.bn6 = nn.BatchNorm1d(numberOfNeurons)
        self.bn7 = nn.BatchNorm1d(numberOfNeurons)
        self.bn8 = nn.BatchNorm1d(numberOfNeurons)
        ...
        # Optimizer
        self.optimizer = optim.Adam(self.parameters(), lr=value_learningRate, weight_decay=L2Factor)
    ...
    def forward(self, input, actions):
        ...
        sa = torch.cat((input, actions), 1)
        x = self.dropout1(F.leaky_relu(self.bn1(self.fc1(sa))))
        x = self.dropout2(F.leaky_relu(self.bn2(self.fc2(x))))
        x = self.dropout3(F.leaky_relu(self.bn3(self.fc3(x))))
        q1 = self.fc4(x)

        x = self.dropout6(F.leaky_relu(self.bn6(self.fc6(sa))))
        x = self.dropout7(F.leaky_relu(self.bn7(self.fc7(x))))
        x = self.dropout8(F.leaky_relu(self.bn8(self.fc8(x))))
        q2 = self.fc9(x)

        return q1, q2

    def Q1(self, input, actions):
        sa = torch.cat((input, actions), 1)
        x = self.dropout1(F.leaky_relu(self.bn1(self.fc1(sa))))
        x = self.dropout2(F.leaky_relu(self.bn2(self.fc2(x))))
        x = self.dropout3(F.leaky_relu(self.bn3(self.fc3(x))))
...
class TD3:
    ...
    VARIABLES:  - device: Hardware specification (CPU or GPU).
                - gamma: Discount factor of the TD3 algorithm.
    ...
    def __init__(self, observationSpace, actionSpace, numberOfNeurons=numberOfNeurons, dropout=dropout, 
                 gamma=gamma, targetNetworkUpdate=targetNetworkUpdate,
        ...
        # Set the Four Deep Neural Networks of the DDPG algorithm (policy and target)
        self.pi = Actor(observationSpace, actionSpace, numberOfNeurons, dropout).to(self.device)
        self.pi_target = Actor(observationSpace, actionSpace, numberOfNeurons, dropout).to(self.device)
        self.pi_noisy = Actor(observationSpace, actionSpace, numberOfNeurons, dropout).to(self.device)
        
        self.q = Critic(observationSpace, actionSpace, numberOfNeurons, dropout).to(self.device)
        self.q_target = Critic(observationSpace, actionSpace, numberOfNeurons, dropout).to(self.device)
        
        hard_update(self.q_target, self.q)
        hard_update(self.pi_target, self.pi)
        ...
        self.noise = OrnsteinUhlenbeckActionNoise(actionSpace)
    ...
    def getNormalizationCoefficients(self, tradingEnv):
        ...
        macds = tradingData['MACD'].tolist()
        ccis = tradingData['CCI'].tolist()
        rsis = tradingData['RSI'].tolist()
        adxs = tradingData['ADX'].tolist(),
        ...
        coeffs = (np.min(macds)/margin, np.max(macds)*margin)
        coefficients.append(coeffs)
        # 6. ccis => minimum and maximum values
        coeffs = (np.min(ccis)/margin, np.max(ccis)*margin)
        coefficients.append(coeffs)
        # 7. rsis => minimum and maximum values
        coeffs = (np.min(rsis)/margin, np.max(rsis)*margin)
        coefficients.append(coeffs)  
        # 8. adxs => minimum and maximum values
        coeffs = (np.min(adxs)/margin, np.max(adxs)*margin)
        coefficients.append(coeffs)      
        return coefficients
    ...
    def processState(self, state, coefficients):
        ...
        macds = [state[4][i] for i in range(len(state[4]))]
        ccis = [state[5][i] for i in range(len(state[5]))]
        rsis = [state[6][i] for i in range(len(state[6]))]
        adxs = [state[7][i] for i in range(len(state[7]))]
        ...
        # 5. Macds => MinMax normalization
        macds = [macds[i] for i in range(1, len(macds))]
        if coefficients[4][0] != coefficients[4][1]:
            state[4] = [((x - coefficients[4][0])/(coefficients[4][1] - coefficients[4][0])) for x in macds]
        else:
            state[4] = [0 for x in macds]        

        # 6. ccis => MinMax normalization
        ccis = [ccis[i] for i in range(1, len(ccis))]
        if coefficients[5][0] != coefficients[5][1]:
            state[5] = [((x - coefficients[5][0])/(coefficients[5][1] - coefficients[5][0])) for x in ccis]
        else:
            state[5] = [0 for x in ccis]        

        # 7. rsis => MinMax normalization
        rsis = [rsis[i] for i in range(1, len(rsis))]
        if coefficients[6][0] != coefficients[6][1]:
            state[6] = [((x - coefficients[6][0])/(coefficients[6][1] - coefficients[6][0])) for x in rsis]
        else:
            state[6] = [0 for x in ccis]        

        # 8. adxs => MinMax normalization
        adxs = [adxs[i] for i in range(1, len(adxs))]
        if coefficients[7][0] != coefficients[7][1]:
            state[7] = [((x - coefficients[7][0])/(coefficients[7][1] - coefficients[7][0])) for x in adxs]
        else:
            state[7] = [0 for x in adxs]       
...
#            self.pi_target.load_state_dict(self.pi.state_dict())
        ...
        soft_update(self.pi_target, self.pi, tau)                     
    ...
    def chooseAction(self, state):
        ...
        with torch.no_grad():
            hard_update(self.pi_noisy, self.pi)
            actor_params = self.pi_noisy.state_dict()

            param = actor_params['fc4.bias']
    ...
    def learning(self, batchSize=batchSize):
        ...
        if (len(self.replayMemory) >= batchSize):
            ...
            with torch.no_grad():        
                ...
                noise = torch.randn(len(action)) * 0.1
                nextActions = (self.pi_target(nextState) + torch.tensor(np.stack([noise, -noise], axis=1), device = self.device)).clamp(0, 1)
                q1_target, q2_target = self.q_target(nextState, nextActions)
                nextQValues = torch.min(q1_target, q2_target).squeeze(1)
                expectedQValues = reward + gamma * nextQValues * (1 - done)

            # Compute the current Q values returned by the critic network
            q1_batch, q2_batch = self.q(state, action)
           
            # Compute the Huber loss
            value_loss = F.smooth_l1_loss(q1_batch.squeeze(1), expectedQValues) + F.smooth_l1_loss(q2_batch.squeeze(1), expectedQValues)
            ...
            # Gradient Clipping
            torch.nn.utils.clip_grad_norm_(self.q.parameters(), gradientClipping)

            # Perform the Deep Neural Network optimization
            self.q.optimizer.step()


            # delayed policy gradient update
            if(self.iterations % self.targetNetworkUpdate == 0): 
                actions = self.pi(state)
                policy_loss = -self.q.Q1(state, actions).mean()
    ...
    def training(self, trainingEnv, trainingParameters=[],
        ...
        try:
            ...
            for episode in tqdm(range(trainingParameters[0]), disable=not(verbose)):
                ...
                for i in range(len(trainingEnvList)):
                    ...
                    previousAction = np.array([0.0, 1.0])
                    done = 0
                    stepsCounter = 0
                    ...
                    while done == 0:
                        ...
                        nextState, reward, done, info = trainingEnvList[i].step(np.argmax(action))
                        ...
                        # Execute the TD3 learning procedure
                        stepsCounter += 1
                        if stepsCounter == learningUpdatePeriod:
                            value_loss, policy_loss = self.learning()
       ...
       # rendoring loss
        ...
        plt.savefig(''.join(['Figures/', str(marketSymbol), '_TD3_TrainingLoss', '.png']))
        ...
        if plotTraining:
            ...
            plt.savefig(''.join(['Figures/', str(marketSymbol), '_TD3_TrainingTestingPerformance', '.png']))
            #plt.show()
            for i in range(len(trainingEnvList)):
        ...
        if showPerformance:
            analyser = PerformanceEstimator(trainingEnv.data)
            analyser.displayPerformance('DDPG')
    ...
    def testing(self, trainingEnv, testingEnv, rendering=False, showPerformance=False):
        ...
        self.noise.scale = 0.0   # turn off randomness
        ...
        while done == 0:
            ...
            # Choose an action according to the RL policy and the current RL state
            action = self.chooseAction(state)
            tensorState = torch.tensor(state, dtype=torch.float, device=self.device)
            tensorAction = torch.tensor(action, dtype=torch.float, device=self.device)
            q0value, q1value = self.q(tensorState.unsqueeze(0), tensorAction.unsqueeze(0))
            # Interact with the environment with the chosen action
            nextState, _, done, _ = testingEnvSmoothed.step(np.argmax(action))
            testingEnv.step(np.argmax(action))
                
            # Update the new state
            state = self.processState(nextState, coefficients)

            # Storing of the Q values
            QValues0.append(q0value.cpu().detach().numpy().squeeze(0))
            QValues1.append(q1value.cpu().detach().numpy().squeeze(0))
        ...
        if showPerformance:
            analyser = PerformanceEstimator(testingEnv.data)
            analyser.displayPerformance('TD3')
    ...
    def plotTraining(self, score, marketSymbol):
        ...
        ax1.plot(score)
        plt.savefig(''.join(['Figures/', str(marketSymbol), '_TD3_TrainingResults', '.png']))
        #plt.show()
    ...
    def plotQValues(self, QValues0, QValues1, marketSymbol):
        ...
        plt.savefig(''.join(['Figures/', str(marketSymbol), '_TD3_QValues', '.png']))
        #plt.show()
    ...
    def plotExpectedPerformance(self, trainingEnv, trainingParameters=[], iterations=10):
        ...
        qinitialWeights =  copy.deepcopy(self.q.state_dict())
        ...
        try:
            ...
            for iteration in range(iterations):
                ...
                if iteration < (iterations-1):
                    trainingEnv.reset()
                    testingEnv.reset()
                    self.q.load_state_dict(qinitialWeights)
        ...
        for i in range(iteration):
            ...
            ax.legend(["Training", "Testing"])
            plt.savefig(''.join(['Figures/', str(marketSymbol), '_TD3_TrainingTestingPerformance', str(i+1), '.png']))
            #plt.show()
        ...
        ax.legend(["Training", "Testing"])
        plt.savefig(''.join(['Figures/', str(marketSymbol), '_TD3_TrainingTestingExpectedPerformance', '.png']))
        #plt.show()
    ...
    def saveModel(self, fileName):
        ...
        torch.save({
                    'model_q_state_dict' : self.q.state_dict(),
    ...
    def loadModel(self, fileName):
        ...
        self.q.load_state_dict(checkpoint['model_q_state_dict'])
        self.q_target.load_state_dict(self.q.state_dict())
    ...
    def plotEpsilonAnnealing(self):
        ...
        plt.savefig(''.join(['Figures/', '_TD3_EpsilonAnnealing', '.png']))
        #plt.show()
